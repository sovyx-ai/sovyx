...
Dashboard tem **zero gaps críticos** e **100% type alignment** entre backend e frontend. Apenas ajuste menor: falta `import "@/lib/i18n"` em `dashboard/src/main.tsx`.
...
## ⚠️ Top 5 divergências (código diverge da spec)
...
| 3 | **Stripe Connect webhook só 6 eventos (planejado 20+)** | cloud | IMPL-011 §2 | Eventos não-cobertos não são processados |
...
**Gap único actionable**: falta `import "@/lib/i18n"` em `dashboard/src/main.tsx` (1 linha).
...
- [ ] `dashboard/src/main.tsx`: adicionar `import "@/lib/i18n"`
