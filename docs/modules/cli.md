...
cli/
  ...
  └── commands/
        ├── brain_analyze.py   sovyx brain {search|stats|analyze}
        ├── dashboard.py       sovyx dashboard {start|stop|token}
        ├── logs.py            sovyx logs {tail|search}
...
| `sovyx doctor` | 10+ diagnósticos (via `upgrade/doctor.py`) | Aligned |
| `sovyx brain search` | Busca hybrid na Brain | Aligned |
| `sovyx brain stats` | Métricas de Brain | Aligned |
| `sovyx brain analyze` | Análise de concepts/relations | Aligned |
...
| `sovyx dashboard start/stop` | Gerencia dashboard | Aligned |
| `sovyx logs tail/search` | Stream/grep de logs JSON | Aligned |
...
| `sovyx doctor` (integra `upgrade/doctor.py`) | Aligned |
| Comandos core (init/start/stop/status) | Aligned |
| Comandos brain/mind/plugin/dashboard/logs | Aligned |
...
- `src/sovyx/cli/commands/brain_analyze.py` — `brain analyze` subcommand.
- `src/sovyx/cli/commands/dashboard.py` — `dashboard start/stop`.
- `src/sovyx/cli/commands/logs.py` — `logs tail/search`.
