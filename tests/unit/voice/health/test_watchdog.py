...
class TestHotplugReaction:
    ...
    @pytest.mark.asyncio()
    async def test_default_device_changed_is_currently_ignored(self) -> None:
        """Sprint 2 Task #18 wires this handler; Sprint 2 Task #17 leaves it
        as the documented no-op path (debug log, no state change)."""
        rc = _ReCascadeRecorder()
        wd, _, _, _ = _make_watchdog(re_cascade=rc)
        listener = _FakeHotplug()
        await wd.start(listener)
