...
38. **Lazy `from X import Y` inside functions invalidates module-level test patches; patch the SOURCE module + handle platform-specific attrs with `create=True`:** when a function does `from X import Y` at call-time (deferred / circular-avoidance / optional-dep handling), the patch path that works for module-level imports does NOT work. Forensic case (Mission `MISSION-voice-calibration-extreme-audit-2026-05-06.md` rc.4 Agent 3 #11): `_run_voice_calibrate_evaluate_rules` at `cli/commands/doctor.py:859-863` does `from sovyx.voice.calibration import capture_fingerprint, CalibrationEngine` and `from sovyx.voice.calibration._measurer import capture_measurements` INSIDE the function body. Patching `sovyx.cli.commands.doctor.capture_fingerprint` (the doctor module's top-level binding) does NOT affect the lazy import — it resolves the attribute on the SOURCE package at call-time. Tests MUST patch `sovyx.voice.calibration.capture_fingerprint` (the source module's attribute) so the lazy `from sovyx.voice.calibration import capture_fingerprint` picks up the patched value. The general rule: **for every patch target, identify whether the binding is module-level (eager `import` at the top of the file → patch the module's attribute) or lazy (`import` inside a function body → patch the SOURCE module's attribute)**. Mixed cases are common: a single test may patch BOTH `sovyx.cli.commands.doctor.triage_tarball` (eager-imported at `doctor.py:67-73`) AND `sovyx.voice.calibration.capture_fingerprint` (lazy-imported inside the function). Extends anti-pattern #20 (patches must follow module splits) to lazy-import boundaries. **Cross-platform corollary**: when production code references a platform-specific attribute (e.g. `signal.SIGKILL` exists on POSIX but NOT on Windows), tests that patch `sys.platform` to "linux" on a Windows host MUST also inject the missing attribute via `patch.object(target, "ATTR", value, create=True)` — else the patched code path crashes on `AttributeError` BEFORE the production logic runs. The rc.4 POSIX cancellation tests at `tests/unit/voice/diagnostics/test_runner.py:715` use `patch.object(_runner.signal, "SIGKILL", _SIGKILL_VALUE, create=True)` with the canonical POSIX value 9 to make the cross-platform test work uniformly. Same pattern applies to `os.killpg` (`create=True` because the attribute doesn't exist on Windows) and any other POSIX-only stdlib symbol the production code references inside a `sys.platform != "win32"` guard.
