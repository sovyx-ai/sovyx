...
- Não implementado no Sovyx ainda — marcado `[NOT IMPLEMENTED]` em v0.6 backlog (ver `SOVYX-BKD-SPE-007-LLM-ROUTER` §Cost Tracking + caching strategy; implementação atual só usa TTL simples no lado do cliente).
