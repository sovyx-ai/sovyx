...
6. **Voice** — "Knows who's speaking" (ECAPA-TDNN roadmap — `[NOT IMPLEMENTED]` em v0.5; v0.6 — ver `SOVYX-BKD-IMPL-005-SPEAKER-RECOGNITION` spec).
7. **Universal** — "Works on any hardware, any LLM".

Configuração típica que cobre 1-3 e 7 simultaneamente (extraído de `mind.yaml` canonical):

```yaml
# ~/.sovyx/minds/default/mind.yaml — local-first + BYO-provider
name: default
llm:
  provider: ollama            # 100% local, sem chamada externa
  model: llama3.1:8b-instruct
brain:
  retention_days: unlimited   # memória persistente — diferencial #1
  scoring:
    weights:
      category_base: 0.15
      llm_assessment: 0.35
