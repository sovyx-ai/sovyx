...
- **Não implementado explicitamente**: a equação `B_i = ln(...)` — Sovyx usa importance acumulativa com decay aplicado por job, não cálculo por-query. `[NOT IMPLEMENTED]` em v0.6 exploratory backlog (ver `SOVYX-BKD-IMPL-002-BRAIN-ALGORITHMS` §3 e `SOVYX-BKD-SPE-004-BRAIN-MEMORY` pro contrato atual de scoring).
...
Modelo real (extraído de `src/sovyx/brain/models.py`):
...
class Concept:
    ...
    importance: float          # 0.0–1.0 — scoring composto
    confidence: float          # 0.0–1.0 — multi-signal
    access_count: int          # base-level activation (ACT-R)
    last_accessed: datetime    # pra decay temporal (Ebbinghaus)
    emotional_valence: float   # 1D (ADR-001 decidiu 3D PAD — divergence)
    source: ConceptSource
    embedding: bytes | None    # ONNX Runtime output
```
