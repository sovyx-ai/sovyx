...
(Moonshine v2 STT via moonshine-voice, Piper/Kokoro TTS, SileroVAD v5) com auto-seleção
...
- `SOVYX-BKD-IMPL-004-VOICE-ONNX.md` — API moonshine (spec original em v1.0; código usa v2 via lib moonshine-voice — breaking change declarada no próprio spec §2.1),
...
- **Moonshine STT** (`stt.py`): ONNX Runtime via lib `moonshine-voice` (v2 API — breaking change declarada em IMPL-004 §2.1).
...
| Classe | Descrição |
|---|---|
| `VoicePipeline` | Orquestrador do pipeline de voz — state machine IDLE → WAKE → RECORDING → ... → IDLE. |
| `AudioOutputQueue` | Output queue de áudio com suporte a interrupção (barge-in). |
| `BargeInDetector` | Detecta fala do usuário durante TTS (threshold ~160ms sustentado). |
| `SovyxWyomingServer` | Servidor TCP Wyoming com descoberta opcional via Zeroconf (mDNS). |
| `WyomingClientHandler` | Handler de conexão Wyoming individual (handshake info + roteamento). |
| `WyomingEvent` | Evento do protocolo Wyoming (JSONL header + payload PCM opcional). |
| `MoonshineSTT` | STT Moonshine v2 via biblioteca moonshine-voice sobre ONNX Runtime. |
| `CloudSTT` | STT cloud via OpenAI Whisper API (BYOK) para fallback. |
| `STTEngine` | Base abstrata para engines speech-to-text. |
| `TranscriptionResult` | Resultado de transcrição de utterance completa. |
| `PartialTranscription` | Transcrição incremental do modo streaming. |
| `TranscriptionSegment` | Segmento dentro de uma transcrição. |
| `PiperTTS` | TTS Piper — síntese local rápida via VITS ONNX. |
| `KokoroTTS` | TTS Kokoro-82M via wrapper kokoro-onnx, qualidade alta. |
| `TTSEngine` | Base abstrata para engines text-to-speech. |
| `SileroVAD` | SileroVAD v5 com inferência ONNX e state machine de histerese. |
| `VADEvent` | Resultado do processamento de um frame de áudio. |
| `WakeWordDetector` | Detector OpenWakeWord com verificação 2-stage (ONNX + STT). |
| `VerificationResult` | Resultado da verificação stage-2 via STT. |
| `VoiceModelAutoSelector` | Auto-detecta hardware e seleciona modelos ótimos (PI5 / N100 / Desktop / Cloud). |
| `AudioCapture` | Captura real-time de áudio via sounddevice. |
| `AudioOutput` | Output de áudio com priority queue, ducking e normalização LUFS. |
| `AudioDucker` | Reduz volume de áudio de fundo quando o agente fala. |
| `JarvisIllusion` | Injeção de fillers e beeps de confirmação para reduzir latência percebida. |

### Errors

| Exception | Quando é raised |
|---|---|
| `CloudSTTError` | Erro durante transcrição cloud STT (OpenAI Whisper). |

### Events

| Event | Payload / trigger |
|---|---|
| `WakeWordDetectedEvent` | Wake word detectada (confidence + timestamp). |
| `SpeechStartedEvent` | Recording de fala começa (após wake word). |
| `SpeechEndedEvent` | Recording termina (silêncio detectado). |
| `TranscriptionCompletedEvent` | STT produziu transcrição (text + confidence + duration). |
| `TTSStartedEvent` | Playback de TTS começa. |
| `TTSCompletedEvent` | Playback de TTS termina. |
| `BargeInEvent` | Usuário interrompeu TTS. |
| `PipelineErrorEvent` | Erro irrecuperável no pipeline de voz. |

### Configuration

| Config | Campo/Finalidade |
|---|---|
| `VoicePipelineConfig` | Configuração do orquestrador VoicePipeline. |
| `WyomingConfig` | Configuração do servidor Wyoming (port, zeroconf, attribution). |
| `MoonshineConfig` | Configuração do MoonshineSTT (model path, language). |
| `CloudSTTConfig` | Configuração do CloudSTT (OpenAI Whisper API via BYOK). |
| `PiperConfig` | Configuração do PiperTTS (voice model, speaker). |
| `KokoroConfig` | Configuração do KokoroTTS (voice, speed). |
| `VADConfig` | Parâmetros calibrados do SileroVAD v5 (IMPL-004 §5). |
| `WakeWordConfig` | Configuração do detector 2-stage de wake word. |
| `AudioCaptureConfig` | Configuração de AudioCapture (sample_rate, device, frames). |
| `AudioOutputConfig` | Configuração de AudioOutput (priority queue, ducking, LUFS). |
| `JarvisConfig` | Configuração do JarvisIllusion (delay, fillers, beeps). |
| `HardwareProfile` | Capacidades de hardware detectadas (CPU cores, RAM, GPU). |
| `ModelSelection` | Seleção de modelos de voz para um HardwareProfile. |
| `HardwareTier` | Tier de hardware suportado (PI5 / N100 / DESKTOP_CPU / DESKTOP_GPU / CLOUD). |
