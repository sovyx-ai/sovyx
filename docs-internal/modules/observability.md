...
- **[DIVERGENCE] SpanProcessor** — IMPL-015 manda `BatchSpanProcessor` (agrupa spans em batches, baixa overhead, recomendado em produção). O código em `tracing.py:39` usa `SimpleSpanProcessor` (export síncrono, span por span). Funciona pra dev/debug mas tem latência maior e risco de overhead em produção com alto throughput. Migração: trocar por `BatchSpanProcessor` quando o backend OTel estiver estabilizado.

- **[DIVERGENCE] gen_ai semantic conventions** — IMPL-015 adota as semantic conventions de OpenTelemetry para LLMs (`gen_ai.system`, `gen_ai.request.model`, `gen_ai.usage.input_tokens`, etc.). O código atual em `tracing.py` adiciona atributos Sovyx-específicos (`provider`, `model`, `mind_id`) que podem não coincidir 1:1 com o padrão OTel. Risco baixo — dashboards custom funcionam; interoperabilidade com Grafana Cloud/OTel Collector pode exigir mapping.
...
| Classe | Descrição |
|---|---|
| `HealthRegistry` | Registro central de checks; descobre e executa health checks. |
| `HealthCheck` | ABC — contrato de um check (`name`, `run() -> CheckResult`). |
| `CheckResult` | Resultado imutável de um check (status, message, metadata). |
| `CheckStatus` | StrEnum — `green` / `yellow` / `red`. |
| `AlertManager` | Avaliador threshold + integração com event bus e SLO burn rate. |
| `Alert` | Alerta disparado (rule_name, severity, message, current_value). |
| `AlertState` | IntEnum — `FIRING` / `RESOLVED`. |
| `AlertSeverity` | StrEnum — `info` / `warning` / `critical`. |
| `MetricsRegistry` | MeterProvider + 30+ instrumentos OTel (counters, histograms, gauges). |
| `SovyxTracer` | Wrapper OTel Tracer para spans do loop cognitivo. |
| `PrometheusExporter` | Exporter `/metrics` via dashboard. |
| `SLOMonitor` | Monitor dos 5 SLOs core com gerenciamento de trackers. |
| `SLOTracker` | Sliding window + burn rate multi-window (Google SRE). |
| `SLODefinition` | Definição de SLO (target, window, burn_rate rules). |
| `SLOReport` | Snapshot do estado de um SLO. |
| `SLOStatus` | StrEnum — estado do SLO (`healthy` / `warning` / `violated`). |
| `SLOEvent` | Evento registrado pelo tracker (sucesso/falha com timestamp). |
| `BurnRateAlertRule` | Regra multi-window (short/long) para burn rate. |
| `SecretMasker` | Masking estrutural de campos sensíveis em logs. |

`HealthRegistry` descobre automaticamente 10 implementações de `HealthCheck`: `DiskSpaceCheck`, `RAMCheck`, `CPUCheck`, `DatabaseCheck`, `BrainIndexedCheck`, `LLMReachableCheck`, `ModelLoadedCheck`, `ChannelConnectedCheck`, `ConsolidationCheck`, `CostBudgetCheck`.

### Errors
| Exception | Quando é raised |
|---|---|

*(sem exceptions dedicadas — checks retornam `CheckStatus.RED` em falha; logging/metrics são no-op safe)*

### Events
| Event | Payload / trigger |
|---|---|
| `AlertFired` | Transição para FIRING — rule_name, severity, metric_name, current_value, threshold. |
| `AlertResolved` | Transição FIRING → RESOLVED — rule_name, severity, message. |

### Configuration
| Config | Campo/Finalidade |
|---|---|
| `AlertRule` | Definição imutável de regra (name, metric_name, threshold, window_seconds, severity, min_events). |
| `SLODefinition` | Objetivo SLO (target, window, burn_rate rules) — ver SPE-026 §6. |
| `MetricSample` | Ponto de métrica (timestamp monotonic, value) usado pelo AlertManager. |
