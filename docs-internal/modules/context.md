...
- Nenhuma divergência documentada contra `SOVYX-BKD-SPE-006-CONTEXT-ASSEMBLY`. Módulo implementa integralmente os 6 slots, TokenBudgetManager adaptativo, e Lost-in-Middle ordering.
...
| Classe | Descrição |
|---|---|
| `ContextAssembler` | Assembla contexto completo com 6 slots (SPE-006); chamado por ThinkPhase. |
| `AssembledContext` | Contexto pronto para LLM (messages, tokens_used, budget, sources, breakdown). |
| `TokenBudgetManager` | Aloca tokens adaptativamente entre slots conforme conversa/complexity/confidence. |
| `TokenBudget` | Alocação de tokens por slot (system/temporal/concepts/episodes/conversation/response). |
| `TokenCounter` | Conta tokens via tiktoken (cl100k_base) com fallback heurístico chars/4. |
| `ContextFormatter` | Formata dados do brain como texto legível para o contexto LLM. |

### Errors

| Exception | Quando é raised |
|---|---|
| `TokenBudgetError` | Falha na alocação de token budget (context_window muito pequeno). |
