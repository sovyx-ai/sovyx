...
- Contradiction detection: utilities em `contradiction.py` (`ContentRelation`
  enum + função `detect_contradiction()`) são invocadas pelo `BrainService`
  ao persistir concept similar a um existente; `BrainService` emite
  `ConceptContradicted`.
...
- `ImportanceScorer`, `ConfidenceScorer`, `ScoreNormalizer`
  (`scoring.py`, 583 LOC). Três dataclasses de pesos (`ImportanceWeights`,
  `ConfidenceWeights`, `EvolutionWeights`) são configuráveis via
...
- `contradiction.py` (233 LOC) — `ContentRelation` enum (EQUIVALENT /
  COMPATIBLE / CONTRADICTORY / UNRELATED) + funções de módulo
  `detect_contradiction()` / `_detect_via_llm()`. Não é uma classe; é
  uma utility module chamada pelo `BrainService` quando persiste um
  concept similar a existente.
- `BrainService` (`service.py`, 712 LOC) — fachada de alto nível. É quem
  emite `ConceptContradicted` (service.py:254) após invocar
...
| Classe | Descrição |
|---|---|
| `BrainService` | API pública do brain; satisfaz BrainReader + BrainWriter; fachada usada pelo cognitive loop. |
| `ConceptRepository` | Repositório de concepts — CRUD + embedding + search async sobre pool. |
| `EpisodeRepository` | Repositório de episodes — CRUD + embedding + search async sobre pool. |
| `RelationRepository` | Repositório de relations (synapses entre concepts) com weight + co_occurrence. |
| `EmbeddingEngine` | Gera embeddings de texto via E5-small-v2 sobre ONNX Runtime. |
| `ModelDownloader` | Download e verificação de modelos ONNX com resiliência enterprise. |
| `HybridRetrieval` | Search combinado semântico (KNN) + keyword (FTS5) + RRF fusion com quality boost. |
| `SpreadingActivation` | Propagação de ativação de concepts seed para vizinhos via relations (Collins & Loftus 1975). |
| `HebbianLearning` | Fortalece conexões entre concepts co-ativados; star topology cross-turn (K=15). |
| `EbbinghausDecay` | Curva de esquecimento — memórias enfraquecem sem reforço. |
| `WorkingMemory` | Memória de trabalho — concepts atualmente ativos com decay temporal. |
| `ConsolidationCycle` | Manutenção periódica: decay → merge similares → prune abaixo de threshold. |
| `ConsolidationScheduler` | Agenda ConsolidationCycle para rodar periodicamente em background. |
| `ImportanceScorer` | Scoring multi-sinal (5 signals) de importance de concepts. |
| `ConfidenceScorer` | Scoring multi-sinal (4 signals) de confidence de concepts. |
| `ScoreNormalizer` | Impede importance/confidence de convergir para valores uniformes. |
| `Concept` | Concept semântico no neocórtex (pydantic, 15 campos). |
| `Episode` | Memória episódica no hipocampo com valence + arousal 2D. |
| `Relation` | Synapse entre dois concepts no grafo do brain (source/target/type/weight). |

### Configuration

| Config | Campo/Finalidade |
|---|---|
| `ImportanceWeights` | Pesos de scoring inicial de importance (soma == 1.0, validada em __post_init__). |
| `ConfidenceWeights` | Pesos de scoring inicial de confidence (soma == 1.0). |
| `EvolutionWeights` | Pesos para recálculo de importance durante consolidação. |
