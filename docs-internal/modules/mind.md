...
| Classe | Descrição |
|---|---|
| `PersonalityEngine` | Traduz config de personalidade em system prompt LLM (OCEAN + tone + safety + anti-injection). |

### Configuration

| Config | Campo/Finalidade |
|---|---|
| `PersonalityConfig` | Traços de personalidade (tone, formality, humor, assertiveness, curiosity, empathy, verbosity). |
| `OceanConfig` | Big Five (openness, conscientiousness, extraversion, agreeableness, neuroticism). |
| `LLMConfig` | Configuração de provider LLM com auto-detect de API keys em runtime. |
| `ScoringConfig` | Pesos de importance + confidence com validação `sum == 1.0` (tolerância 0.01). |
| `BrainConfig` | Config do brain: consolidation_interval_hours, dream_time, decay_rate, min_strength. |
| `TelegramChannelConfig` | Config do canal Telegram (token_env, chat_ids). |
| `DiscordChannelConfig` | Config do canal Discord. |
| `ChannelsConfig` | Agregação de canais de comunicação. |
| `Guardrail` | Regra de safety guardrail custom (SPE-002) — id, rule, severity, builtin. |
| `CustomRule` | Regra de safety definida pelo owner. |
| `ShadowPattern` | Pattern de safety avaliado em modo shadow/dry-run. |
| `SafetyConfig` | Seção safety do mind.yaml (child_safe_mode, content_filter, guardrails, shadow_mode). |
| `PluginConfigEntry` | Configuração per-plugin (config + permissions) a partir do mind.yaml. |
| `PluginsConfig` | Seção plugins do mind.yaml (enabled/disabled whitelists, tool_timeout_s). |
| `MindConfig` | Configuração completa da mente — modelo raiz do mind.yaml. |
