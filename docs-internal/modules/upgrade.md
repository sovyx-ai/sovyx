...
| Classe | Descrição |
|---|---|
| `Doctor` | Orquestrador dos 10+ diagnósticos do `sovyx doctor`. |
| `DiagnosticResult` | Resultado de um check (check, status, message, fix_suggestion). |
| `DiagnosticReport` | Agregado de todos os checks com overall status. |
| `DiagnosticStatus` | StrEnum — `pass` / `warn` / `fail`. |
| `MindImporter` | Carrega SMF (diretório) ou `.sovyx-mind` (ZIP) em Mind existente/novo. |
| `ImportInfo` | Resultado do import (concepts/episodes/relations importados, warnings). |
| `MindExporter` | Gera SMF para backup manual ou transferência entre instâncias. |
| `ExportManifest` | Manifest do SMF (versão, mind_id, counts, schema_version). |
| `ExportInfo` | Resultado do export (path, size, counts). |
| `BackupManager` | Pre-upgrade snapshot + rollback on failure. |
| `BackupInfo` | Metadata do backup criado (path, trigger, sha256, size). |
| `BackupTrigger` | StrEnum — trigger do backup (`migration` / `daily` / `manual`). |
| `BlueGreenUpgrader` | Estratégia zero-downtime — blue ativo → green novo → swap. |
| `UpgradeResult` | Resultado do upgrade (success, old/new version, rollback_performed). |
| `UpgradePhase` | StrEnum — fases (`install` / `verify` / `swap` / `rollback`). |
| `VersionInstaller` | Instala nova versão em env green isolado. |
| `MigrationRunner` | Runner multi-banco (complementa `persistence/migrations.py`). |
| `UpgradeMigration` | Contrato de migration (version SemVer, up/down, checksum). |
| `MigrationReport` | Agregado de migrations aplicadas. |
| `SchemaVersion` | Versão SemVer rastreada no schema. |
| `SemVer` | Helper para parse/compare de SemVer. |

### Errors
| Exception | Quando é raised |
|---|---|
| `UpgradeError` | Base para erros de upgrade (herda de `MigrationError`). |
| `InstallError` | Instalação da nova versão falhou (download, checksum, deps). |
| `VerificationError` | Health checks do green ambient falharam. |
| `RollbackError` | Rollback para blue falhou — estado instável requer intervenção. |
| `BackupError` | Falha em criar/restaurar snapshot (herda de `PersistenceError`). |
| `BackupIntegrityError` | SHA-256 mismatch — backup corrompido. |
| `ImportValidationError` | SMF/`.sovyx-mind` inválido (schema, versão, checksum). |

### Events
| Event | Payload / trigger |
|---|---|

*(eventos de upgrade não são publicados no event bus — fluxo é síncrono via CLI com output Rich; progresso rastreado em logs estruturados)*

### Configuration
| Config | Campo/Finalidade |
|---|---|
| `ExportManifest` | Manifest do SMF exportado (versão, mind_id, schema_version, counts). |

*(não há `UpgradeConfig` / `ImportManifest` dedicados — config de upgrade é flags de CLI; manifest de import é o próprio SMF lido no disco)*
