...
| Classe | Descrição |
|---|---|
| `BridgeManager` | Orquestrador: routing de InboundMessage, LRU locks, financial confirmation flow. |
| `InboundMessage` | Mensagem normalizada entrando do canal (channel, user_id, text, callback_data). |
| `OutboundMessage` | Mensagem saindo para o canal (text, reply_to, inline_buttons). |
| `InlineButton` | Botão inline com `callback_data` (max 64 bytes — limite Telegram). |
| `PersonResolver` | Mapeia `channel_user_id` para `PersonId` persistente. |
| `ConversationTracker` | Gerencia conversas ativas, TTL e cleanup. |
| `TelegramChannel` | Adapter aiogram 3.x (text + reply; sem media/groups em v0.1). |
| `SignalChannel` | Adapter REST contra `signal-cli-rest-api`. |

*(Apenas `InlineButton`, `InboundMessage` e `OutboundMessage` são re-exportados via `sovyx.bridge.__all__`. Os demais são importados diretamente do submódulo — ex. `from sovyx.bridge.manager import BridgeManager`.)*

### Errors
| Exception | Quando é raised |
|---|---|

*(sem exceptions dedicadas — falhas de canal propagam como `ChannelConnectionError` de `engine.errors`)*

### Events
| Event | Payload / trigger |
|---|---|
| `ChannelConnected` | Canal conectou — `channel_type` (definido em `engine.events`, relevante aqui). |
| `ChannelDisconnected` | Canal desconectou — `channel_type`, `reason`. |
| `PerceptionReceived` | Emitido ao converter InboundMessage → Perception para o CogLoopGate. |

*(MessageReceived/MessageSent não são eventos dedicados — fluxo usa `PerceptionReceived` + `ResponseSent` do engine)*

### Configuration
| Config | Campo/Finalidade |
|---|---|

*(não há dataclass `BridgeConfig` dedicado — canais leem credenciais de `EngineConfig` / env vars)*
