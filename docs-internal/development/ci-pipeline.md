...
- **SOVYX-BKD-SPE-001-ENGINE-CORE** (`vps-brain-dump/.../specs/`) — bootstrap/lifecycle contract — CI health-check jobs validate this.
- **SOVYX-BKD-ADR-004-DATABASE-STACK** — 9 SQLite pragmas, WAL, sqlite-vec — CI must load these correctly for deterministic runs.
- **SOVYX-BKD-IMPL-015-OBSERVABILITY** — logging + tracing pipeline (informs how to debug failing CI logs).
- **`vps-brain-dump/memory/nodes/aiosqlite-deadlock-protocol.md`** — origin node of the 2026-04-13 fix; referenced above.
