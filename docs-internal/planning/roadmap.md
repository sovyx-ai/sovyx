...
Fluxo canônico de release (comando real conforme `CLAUDE.md` §Deploy Flow):

```bash
# 1. Bump version em pyproject.toml + src/sovyx/__init__.py
# 2. Commit + tag + push
git commit -am "chore: bump v0.6.0"
git tag v0.6.0
git push origin main --tags

# 3. CI dispara automaticamente:
#    Lint → Type → Security → Tests → Dashboard Build
#    Tag vX.Y.Z → Docker Build + PyPI Publish
```
